...
| 实验 | 训练数据 | Tokenizer | Val Loss | 累计收益 | 周均 | 夏普 |
|------|----------|-----------|------|------|------|------|
| **Phase 1** | **24-25** | **预训练 (冻结)** | — | **+19.66%** | **+1.44%** | **0.46** |
| Phase 2 | 22-25 | 微调 | 3.73 | +11.54% | +0.86% | 0.44 |
| Exp A | 22-25 | 预训练 (冻结) | 3.09 | +10.31% | +0.78% | 0.40 |
| Exp B | 24-25 | 微调 | 3.77 | +15.68% | +1.14% | 0.65 |

**结论**：
1. **数据范围**：24-25 > 22-25（差异 ~9% 累计收益）
2. **Tokenizer**：预训练 > 微调（差异 ~4-8% 累计收益）
3. **交互效应**：两者叠加（Phase 2）导致最大退步 ~8%
4. **代理目标错配**：Exp A 的 val loss 最低但收益最差 → 自回归 cross-entropy loss 不能衡量 stock ranking 质量
